...
export function useAgentAudioVisualizerAura(
  ...
  useEffect(() => {
    // Drive scale smoothly from the mic volume so the aura breathes with the voice.
    // (No isAnimating guard — that blocked volume from ever applying while the
    //  speaking-state scale animation was still running.)
    if (state === 'speaking' && resolvedVolume > 0) {
      const target = 0.2 + 0.3 * Math.min(resolvedVolume, 1);
      animateScale(target, { duration: 0.1, ease: 'easeOut' });
    }
  }, [
    state,
    resolvedVolume,
